...
- **错题本导出支持「Word 打印稿」**：导成 `.docx` 直接打印成纸质题目，
  导出前选择答案排布——**答案在题目下方**（复习背诵）或**答案在最后一页**
  （先做题再对答案）；选择界面配了一个示意动画，把两种排法直接演出来。
  原有的 `.json` 导出保留（可再导入回猫卷）
...
- **导出落在用户能找到的文件夹里，并在导出后显示该位置**：错题导出（Word 打印稿 / JSON）不再写进应用私有缓存，而是 Android 的
  `<外部存储>/Android/data/包名/files/猫卷导出`、桌面端的`文档/猫卷导出`；
  导出完成、从系统分享面板返回后，提示会写明「保存位置：…」并支持一键复制完整路径
  （缓存目录普通文件管理器进不去，导完就找不着；现在能自己翻出来打印/存档）

- **错题本整页可上下滑动**：筛选栏 / 统计条 / 薄弱知识点面板 / 生成建议 /
  题库卡现在在同一个滚动视图里（此前只有题库列表能滚，知识点一多就把列表
  挤到只剩一两张卡）；底部「全题库重刷」按钮仍固定在底部
- **「薄弱知识点建议」弹窗渲染富文本**：此前把 AI 输出的 markdown 原样显示，
  用户看到的是「## 最优先复习知识点」「**病理学**」这种源码；现走 AI 富文本
  渲染器（标题/加粗/列表/表格），并保留可选中复制
